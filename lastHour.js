      ...
      Object.keys(member.completion_day_level).forEach(day => {
        if (member.completion_day_level[day]['1']) {
          ...
          if (day1Star > lastInvokation) {
            solvedLastHour.push({message:`${nick} klarade dag ${day} del 1 ${day1Star.toDateString()} ${day1Star.toTimeString().split(' ')[0]}`, time: day1Star});
          }
        }
        if (member.completion_day_level[day]['2']) {
          ...
          if (day2Star > lastInvokation) {
            solvedLastHour.push({message: `${nick} klarade dag ${day} del 2 ${day2Star.toDateString()} ${day2Star.toTimeString().split(' ')[0]}`, time: day2Star});
          }
        }
      });
    }
  ...
  const r = {
    leaderboard: leaderboard.map(it => `${it.name} poäng: ${it.points}`).join('\n'),
